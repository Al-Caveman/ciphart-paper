...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $G$         & $\gets \hat M/C/\hat T/L$; total number of segments per
                    lane.\\
    $\hat B$    & $\gets \maxf(\log_2(GL\hat TR), B)$.  this is to ensure
    ...
    $m_i$       & $C$-bytes memory for $i^{th}$ task in the $\hat M$-bytes
                    pad.\\
    $n_l$       & $\gets l\hat TR$; nonce variable for $l^{th}$ lane with at
                    least $64$ bits.\\
    $f$         & $\gets 0$; a flag indicating whether the $\hat
