...
\begin{enumerate}
    ...
    in this case shannon's entropy, $H(k) = 8V$ is used as a measure of
    security against brute-forcing attacks.
...
one may try to empirically estimate $\cost(\hash)$ and $\cost(\enc)$ in
(\ref{eq_cost_passbruteforce}), by analysing the industry of asics in order
to obtain a mapping between the total amount of money that's needed in
order to identify $p$ within a given time frame\footnote{the \emph{scrypt}
paper contains such an attempt.}.
