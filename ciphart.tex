...
this a proof by contradiction that that hashing functions, as well as
encryption functions\footnote{because encryption functions can be used to
...
\begin{enumerate}
    ...
    \item or that i've proven that hashing and encryption functions do not
    exist?
\end{enumerate}
...
\begin{itemize}
    ...
    thanks to the ``perfect lie'' theorem (\cref{theorem_perfect_lie}).

    \item memory-\emph{harder} by utilising space beyond the random-access
    memory, by also utilising the hard-disk; thanks to the ``cacheable
    keys'' discovery (discovery \ref{discov_key_caching}).

    \item faster by using $\hash$ only when compression takes place,
    otherwise using $\enc$; thanks to the ``hashing is only for
...
\begin{tabularx}{\columnwidth}{lX}
$k$ & $K$-bytes key, with $H(\mathcal{O}_p) + \log_2 n_0$ many shannon's
        entropy bits, such that $\log_2 n_0 \ge B$.\\
\end{tabularx}
    ...
    \For{$g \gets 0, 1, \ldots, G-1$}{
        ...
        \uIf{$d \le D$}{\label{ciphart_D_filling}
            ...
            \If{$h \ge H$}{\label{ciphart_D_using}
                $v \gets v \oplus \fread(F, v \bmod (d - V), V)$\;
                $u \gets u + 1$\;
                $h \gets 0$\;
            }
...
of those people who get donated to.  i have once heard bryan
lunduke\footnote{\url{http://lunduke.com}} say, in a very energetic manner,
that it feels like being like people who get
donated\footnote{\url{https://youtu.be/radmjL5OIaA?t=12}}.  so i guess it
probably feels very good.
