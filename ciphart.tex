...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $\hat B$    & $\gets \max\Big(
                        ...
                        \big)
                    \Big)$.  this is to ensure that requested spaces are
                    filled, with at least one more pass over the bytes in
                    the random-access memory, that the internal key $v$ is
                    updated from the hard-disk at least once, thanks to
                    $+H$, and then the updated key is used once,
                    thanks to $+1$.\\
\end{tabularx}
\begin{tabularx}{\columnwidth}{lX}
    $\hat B$    & $\gets \log_2\big((2^{\hat B} \bmod C\hat TL) + C\hat
                    TL\big)$.  this is to reflect the reality that tasks
                    are completed in batches of $L$ many lane segments.\\
...
\subsubsection{caveman's entropy of \emph{ciphart}}
if the adversary has $\hat M$ many random-access memory bytes, and $\hat D$
many hard-disk bytes, then caveman's entropy of \emph{ciphart}'s derived
key is $H(\mathcal{O}_p) + \log_2 \hat B$.

what if the adversary has $\hat M - C$ many random-access memory bytes?
...
be used in the future to update the internal key $v$.
