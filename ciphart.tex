...
order to extract all information about $x$:
\begin{equation}
...
\begin{definition}[bundled specifications set]
    ...
    specifications set of $x$'s specifications set $\mathcal{O}_x = \{s_0,
...
\begin{lemma}[specifications bundling is lossless]
    ...
    $\mathcal{O}_x$, then $H(\mathcal{\hat S}_x) = H(\mathcal{O}_x)$
...
say that we've a password $p$.  we know from \cref{sec_pass_entropy} that
$p$'s specifications set is $\mathcal{O}_p$ and its redundancy-free
specifications set is $\mathcal{O}_p^*$, which can be fully revealed after
asking no less than $\log_2 \mathcal{O}_s^*$ many balanced binary
questions.  i.e. $H(\mathcal{O}_p) = \log_2 \mathcal{O}_s^*$.

say that we've got a key $k_h \gets \hash(p, 8V)$.  what is $k$'s entropy?
the answer is $H(\mathcal{O}_k) = H(\mathcal{O}_p) = \log_2
|\mathcal{O}_p^*|$ for as long as $|\mathcal{O}_p^*| \le 2^{8V}$, because
...
not change.  i.e.  $|\mathcal{O}_k^*| = |\mathcal{O}_p^*|$.  hence, $k$'s
