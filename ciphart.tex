...
\subsection{an unorthodox use}
say that we've got a $8V$ bit key $k \gets \hash(p \Vert s, 8V)$, derived
from password $p$, where $s$ is a salt, and attacker has a cipher-text
block $m_0 \gets \enc(\text{``goodbye, world''}, 0, k)$\footnote{``goodbye,
world'' is to be used ever since \emph{dennis ritchie} passed away.  the
colourful ``Hello, world!'', including the capitalised first letter, was
only valid when he was alive.  despite his achievements, he died unnoticed
by almost everyone, as a friend found him dead in his house a week after he
was dead already.  steve jobs, who did almost nothing really useful, got
loads of attention, some of whom even called him \emph{the messiah of
computing}.  this is wrong, and must change.  henceforth i propose: all
``Hello, world!''s to become ``goodbye, world''s, and all
caps, except constants, to be lowered and remain so to eternity.  using
caps for other than constants is useless anyway.}.  attacker's goal is to
eventually decrypt the sequence $m_0, m_1, \ldots$ back to original
plain-text by using $\enc$'s corresponding $\dec$ function.  the attacker
has these options:
\begin{enumerate}
    \item\label{dumbbruteforce} try to decrypt $m_0$, each time with a new
    key, until it results in ``goodbye, world''.  i.e. test whether
    $\text{``goodbye, world''} = \dec(m_0, 0, k)$ is true.  this way the
    attacker will find $k$ but not $p$.  this will require him to perform
    $2^{8V}$ $\dec$ calls in order to guarantee finding $k$.  or,
    $2^{8V}/2$ on average\footnote{in the limit as our repetition of this
    experiment, except for using different passwords, approaches
    infinity.}.  both of which are insanely slow and would take many
    centuries to complete.  e.g. my laptop would take $4.28 \times 10^{58}$
    many centuries in order to increment a mere counter for $2^{8V}$ times.
    so this is not practical.

    adversary's cost\footnote{$\cost(\enc)$ is the cost of calling $\enc$,
    and $\cost(\text{if } c_0 = \text{``goodbye, world''})$ is that of
    testing whether $c_0 \gets \dec(m_0, 0, \hat k)$ equals the expected
    clear-text ``goodbye, world''.} in this option is:
    \begin{equation}\label{eq_cost_keybruteforce}
        2^{8V} \left(
            \begin{split}
                & \cost(\dec) \\
                & + \cost(\text{if } c_0 = \text{``goodbye, world''})
            \end{split}
        \right)
    \end{equation}

    \item if $\enc$ is broken enough by cryptanalysis, then using approach
    in \cref{dumbbruteforce} could become feasible by finding a much
    cheaper decryption function $\hat\dec$ than $\dec$.  but only an idiot
    would use a broken $\enc$ that has $\hat\dec$, and such people need a
    different type of help than what i try to offer here.  so this option
    is off-topic.

    \item\label{passbruteforce} explore key candidates by trying different
    passwords.  while each step is more expensive than those in
    \cref{dumbbruteforce} as it also requires calling $\hash$ before
    $\dec$, the total number of iterations is $2^{H(p)}$, which is usually
    small enough to make this strategy practically viable for the adversary
    to bet on.  in this paper i want to enhance our security against this
    option.  there are sub-scenarios:
    \begin{enumerate}
        \item attacker doesn't know $k$.  here, every time the attacker
        calls $\hash$ to generate a key from a candidate password, he also
        needs to call $\dec$ in order to see if it leads to output
        ``goodbye, world'' so that he knows that the key that he found is
        actually $k$, so that he goes on to decrypt $m_0, m_1, \ldots$.
        his cost\footnote{where $\cost(\hash)$ is the cost of a single call
        to $\hash$.} in this sub-scenario is:
        \begin{equation}\label{eq_cost_passbruteforce_wdec}
            2^{H(p)} \left(
                \begin{split}
                & \cost(\hash) + \cost(\dec) \\
                & + \cost(\text{if } c_0 = \text{``goodbye\ldots''})
                \end{split}
            \right)
        \end{equation}

        \item attacker knows $k$.  here, he doesn't need to call $\dec$, so
        his cost\footnote{where $\hat k \gets \hash(p \Vert s, 8V)$.} is:
        \begin{equation}\label{eq_cost_passbruteforce_wodec}
            2^{H(p)} \left(
...
\subsubsection{calculating the security}
\begin{definition}
the security of a system is the cost of the cheapest method that can break
it.
\end{definition}

i think ways to go forward, to make total costs in
(\ref{eq_cost_keybruteforce}), (\ref{eq_cost_passbruteforce_wdec}) and
(\ref{eq_cost_passbruteforce_wodec}) calculable so that we can get an
understanding of the quantity of security that we have, are:
\begin{itemize}
    \item estimate the $\cost$ function.  e.g. by surveying the asics
    industry to get an idea how much money it costs to get a given key, or
    password, space brute-forced within a target time frame\footnote{see
    the \emph{scrypt} paper for an example.}.

    this has expensive housekeeping as one needs to keep a constant eye on
    the industry of asic manufacturing.  plus it is too heuristic-y to
    build security on.  until i learn more about the theoretical cost
    guarantees about the involved function calls, i'm going to ignore this
    option.

    not saying that it's a bad idea.  in fact i think it's the most
    accurate approach to measure security, as i think security against
    breakage \emph{is} the cost of breaking it.  it is just that this
    approach seems to be too expensive for most people to adopt.

    \item ignore anything that has no cryptographic guarantee.  so, in
    (\ref{eq_cost_keybruteforce}), cryptography
    guarantees\footnote{statistically by confidence earned through peer
    review and attempts to break encryption algorithms.} that $2^{8V}$ many
    tests are needed to be performed in order to find the key $k$ with a
    probability of $1$.

    but there is no cryptographic guarantee on how slow calling
    $\dec$ is across hardware implementations, other than that it will cost
    a unit of time greater than zero.  this way
    (\ref{eq_cost_keybruteforce}) becomes just:
    \begin{equation}\label{eq_simplecost_keybruteforce}
        \begin{split}
                & 2^{8V} \times 1\\
            ={} & 2^{8V}
        \end{split}
    \end{equation}
    likewise, (\ref{eq_cost_passbruteforce_wdec}) and
    (\ref{eq_cost_passbruteforce_wodec}) become just:
    ...
    until i learn more about the guarantees of the involved function calls,
    i'm adopting this conservative thinking in this paper.
\end{itemize}

\begin{request}
    drop me a mail if you know other ways to go forward, even if not
    necessarily related to this paper's conclusions or its style of
    thinking.
\end{request}

i think \cref{eq_simplecost_keybruteforce,eq_simplecost_passbruteforce} is
the reason it made sense to people to use entropy as a measure of password
security.  they just look at it in $log_2$ scale.  i.e.
\cref{eq_simplecost_keybruteforce,eq_simplecost_passbruteforce} become:
    \begin{equation}\label{eq_simplecost_keybruteforce_logscale}
        \begin{split}
                & 2^{8V} \times 1\\
            ={} & 2^{8V}
        \end{split}
    \end{equation}
    and
    \begin{equation}\label{eq_simplecost_passbruteforce_logscale}
        \begin{split}
                & 2^{H(p)} \times 1\\
            ={} & 2^{H(p)}
        \end{split}
...
but this simple $1$ to $1$ mapping between cost and entropy is broken when
expensive key derivation functions is used.  say that the key $k$ was
obtained by recursively calling the $\hash$ function $N$ many times.  i.e.
$\hash(\hash(\ldots\hash(p \Vert s, 8V), \ldots, 8V), 8V)$.  this way
