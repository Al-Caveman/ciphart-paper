...
\DeclareMathOperator{\fexists}{exists}
\DeclareMathOperator{\fread}{read}
\DeclareMathOperator{\fseekread}{seekread}
...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $\fread$    & hard-disk file reading function.\\
...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $s$         & $\gets 0$.  a counter indicating number of processed lane
                    segments since the last disk-read.\\
...
\begin{tabularx}{\columnwidth}{lX}
$k$ & $K$-bytes key.\\
$N$ & total number of times $\enc$ was actually called.  under the worst
        case scenario\footnote{adversary has $\hat M + \hat D$ space.},
        total number of shannon's entropy bits that were injected into $k$
        by \emph{ciphart} is $\log_2 N$ many bits, where $\log_2 N \ge B$.\\
\end{tabularx}
...
\If{$Y$ {\bf and} $\fexists(Z)$}{
    $k, N = \fread(Z, V + \sizeof(N))$\;
    \Return{$k, N$}
}
$i=0, j=1$\;
\For{$0, 1, \ldots, \hat DC^{-1}-1$}{
    $m_j \gets \enc(m_i, n_0, *v)$\;
    $\fwrite(F, m_j)$\;
    $n_0 \gets n_0 + 1$\;
    $\hat i \gets i$\;
    $i \gets j$\;
...
\While{$1$}{
    \For{$g \gets 0, 1, \ldots, G-1$}{
        ...
        $s \gets s + L$\;
        \If{$s \ge S$}{
            $*v \gets *\fseekread(F, v \bmod (\hat D - V), V)$\;
            $s \gets 0$\;
        }
        \If{$f \ge 2$ \bf and $N \ge 2^B$}{
            $g_{\text{last}} \gets g$\;
...
\appendix
\section{donations}
not a big deal, but it makes me happier when i see donations arriving into
my \emph{bitcoin}'s wallet:
\begin{center}
