...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $G$         & $\gets \hat MC^{-1}\hat T^{-1}L^{-1}$.  total number of
                    segments per lane.\\
    $\hat B$    & $\gets 0$.  actual number of added \emph{relative entropy
                    bits}, where $\hat B \ge 2^B$.\\
...
\For{$l=1, 2, \ldots, L-1$}{\label{ciphart_out}
    $i \gets C\times\left(g_{\text{last}}LT + lT + T-1\right)$\;
    $m_i \gets \enc(m_{i - 1}, n_0, v)$\;
    $n_0 \gets n_0 + 1$\;
    $v \gets m_i \bmod(i + C - V)$\;
    $\hat B \gets \hat B + 1$\;
}
\Return{$k, \hat B$}
\caption{ciphart}
