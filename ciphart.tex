...
i think this is why people use password entropy as a measure of its
security.  not because it is the quantity of security, but rather because
its the quantity of \emph{simplified} security.  further, for convenience,
it seems that people report it in the $\log_2$ scale.  i.e. $\log_2
2^{H(p)} = H(p)$.
...
\begin{equation}
    C(\hat p, \hash(\ldots)) = H(p) + \log_2 N
\end{equation}

\begin{equation}
    \begin{split}
        C(p, \rhash(\ldots, N)) &= H(p) + \log_2 N \\
                                &= H(\hat p) \\
    \end{split}
\end{equation}
...
bites such that, as available memory is linearly reduced from $M$, penalty
...
\begin{equation}
    \mhash(p \Vert s, N, M)  =  \rhash(p \Vert s, N)^{e(\hat M - M)}
\end{equation}

if the $\hash$ function is replaced by $M$ bytes memory-hardened $N$-deep
recursion hash function $\mhash$, (\ref{eq_cost_passbruteforce}) becomes:
...
\begin{equation}
    C(p, \mhash(\ldots, N, M)) = H(p) + e(M-\hat M)\log_2 N
\end{equation}
...
\begin{theorem}
when $p$ is secret and $\hash$ is not broken, caveman's entropy $C$ equals
shannon's entropy $H$.
\end{theorem}
