...
\begin{equation}\label{eq_simplecost_passbruteforce_N}
    ...
    \end{split}
\end{equation}
and the $\log_2$-scaled version becomes $H(p) + \log_2 N$.
...
\While{$1$}{
    \For{$g \gets 0, 1, \ldots, G-1$}{
        ...
        \uIf{$d \le D$}{\label{ciphart_D_filling}
            ...
            \If{$f \ge 1$ \bf and $q \ge 1$ \bf and $n \ge 2^B$}{
                $g_{\text{last}} \gets g$\;
...
\appendix
\section{donations}
this work is sponsored by a bright nerdiness that sparked unexpectedly.
nothing here was supposed to happen.  the cause  remains unknown, so we
call it \emph{something random with large entropy}.
