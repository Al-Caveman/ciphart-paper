...
``Hello''s and ``hello''s in algorithms to become ``goodbye''s, and all
...
\begin{enumerate}
    ...
    experiment, except for using different $p$s, approaches infinity.}.
    both of which are insanely slow and would take many centuries to
    complete.  e.g. my laptop would take $4.28 \times 10^{58}$ many
    centuries in order to increment a mere counter for $2^{8V}$ times.  so
    ...
    \begin{enumerate}
        ...
        his cost in this sub-scenario is\footnote{where $\cost(\hash)$ is
        the cost of a single call to $\hash$, $\cost(\enc)$ is that of
        $\enc$, and $\cost(\text{test if } c_0 = \text{``goodbye, world''}$
        is that of testing whether $c_0 \gets \dec(m_0, 0, \hat k)$ equals
        the expected clear-text ``goodbye, world''.}:
        \begin{equation}\label{eq_cost_passbruteforce_wdec}
            2^{H(p)} \left(
                \begin{split}
                & \cost(\hash) \\
                & + \cost(\dec) \\
                & + \cost(\text{test if } c_0 = \text{``goodbye, world''})
                \end{split}
            \right)
        \end{equation}

        \item attacker knows $k$.  here, he doesn't need to call $\dec$, so
        his cost is\footnote{where $\hat k \gets \hash(p \Vert s, 8V)$.}:
        \begin{equation}\label{eq_cost_passbruteforce_wodec}
            2^{H(p)} \left(
                \cost(\hash) + \cost(\text{test if } \hat k = k)
            \right)
        \end{equation}
    \end{enumerate}
\end{enumerate}
