...
\begin{tabularx}{\columnwidth}{lX}
    ...
    $G$         & $\gets \hat MC^{-1}\hat T^{-1}L^{-1}$.  total number of
                    segments per lane.\\
    $\hat B$    & $\gets \maxf(GLT, B - (B\bmod LT) + LT)$.  actual number
                    of times $\enc$ is called so that we at least one pass
                    is performed over the $\hat M$-bytes pad, where $\hat B
                    \ge 2^B$.\\
...
attacks by $\log_2 \hat B$ \emph{relative entropy bits}.\\
\end{tabularx}
